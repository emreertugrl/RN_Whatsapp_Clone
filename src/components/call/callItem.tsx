...
import {ArrowRight2, Call, CallSlash} from 'iconsax-react-nativejs';
        ...
        <View style={styles.middle}>
          <View style={styles.rigthTop}>
            <Text
              style={[
                styles.rightName,
                {color: item.status == 0 ? Colors.RED_1 : Colors.BLACK},
              ]}>
          ...
          <View style={styles.rightDown}>
            {setStatus() ? (
              <Call size={20} color={Colors.GREEN_3} />
            ) : (
              <CallSlash size={20} color={Colors.RED_1} variant="Bold" />
            )}
            <Text style={styles.unreadText}>
              {item.from === phoneNumber ? 'Outgoing' : 'incoming'}
            </Text>
          </View>
        </View>
